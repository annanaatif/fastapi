...
def test_get_all_posts(authorized_client, test_posts, test_user):
    ...
    user_posts = [p for p in test_posts if p.owner_id == test_user['id']]
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "title five", "content": "content five"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "title five"
    assert created_post.content == "content five"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_create_post(client, test_user, test_posts):
    res = client.post("/posts/", json={"title": "title five", "content": "content five"})
    assert res.status_code == 401
